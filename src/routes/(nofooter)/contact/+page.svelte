<div class="flex flex-col gap-16 margins">
	<!-- Title -->
	<div class=" flex flex-col gap-3">
		<h1 class="text-h2 md:text-h1 text-center mt-12">Let’s collaborate!</h1>
	</div>

	<!-- Body -->
	<div class="flex flex-col mb-12 gap-16">
		<!-- Description -->
		<div class="flex flex-col md:flex-row justify-end gap-4 md:gap-4">
			<p class="text-body-base w-full md:w-1/3">
				I’m looking for freelance book design projects and full-time UX/UI opportunities across
				Europe.
			</p>
			<p class="text-body-base w-full md:w-1/3">
				I’m currently based between the Canary Islands and Tübingen, Germany. I’m also willing to
				relocate for the right opportunity.
			</p>
			<p class="text-body-base w-full md:w-1/3">
				If you think we’d be a good match, please feel free to reach out!
			</p>
		</div>

		<!-- Contact information -->
		<div class="flex flex-col gap-4">
			<!-- Divider -->
			<div class="h-[1.5px] bg-[#1040b0]" />

			<!-- Email link -->
			<div class="flex flex-col md:flex-row gap-4 items-end">
				<p class="text-body-sm w-full md:w-1/3">E-mail</p>
				<div class="text-h4 w-full md:w-2/3">
					<a href="mailto:sofpolette@gmail.com" rel="external" class="hover:underline"
						>sofpolette@gmail.com</a
					>
				</div>
			</div>

			<!-- Divider -->
			<div class="h-[1.5px] bg-[#1040b0]" />

			<!-- Social media links -->
			<div class="flex flex-col md:flex-row gap-4 items-end">
				<p class="text-body-sm w-full md:w-1/3">Social Media</p>
				<div class="flex flex-col md:flex-row gap-2 md:gap-8 w-full md:w-2/3 text-h4">
					<a href="https://www.linkedin.com/in/sofiajpolettestubb" class="hover:underline"
						>LinkedIn</a
					>
					<a href="https://www.behance.net/sofapolette1" class="hover:underline">Béhance</a>
					<a href="https://www.instagram.com/jacpolstu/" class="hover:underline">Instagram</a>
				</div>
			</div>

			<!-- Divider -->
			<div class="h-[1.5px] bg-[#1040b0]" />

			<!-- Writing -->
			<div class="flex flex-col md:flex-row gap-4 items-end">
				<p class="text-body-sm w-full md:w-1/3">Writing</p>
				<div class="text-h4 w-full md:w-2/3">
					<a href="https://medium.com/@sofpolette" class="hover:underline">Medium</a>
				</div>
			</div>

			<!-- Divider -->
			<div class="h-[1.5px] bg-[#1040b0]" />
		</div>
	</div>
</div>
